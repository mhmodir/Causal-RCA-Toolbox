...
### Causality Analysis & Root-Cause Fault Identification for Industrial Process Data
...
Six sequential stages. Stages 1–3 build the causal graph, Stage 4 monitors it over time, Stage 5 identifies the root cause, Stage 6 validates and exports.
...
| Constraint | Effect |
| --- | --- |
| Forbidden links | Rules out physically impossible directions (e.g. downstream → upstream) |
...
| Lagged cross-correlation | Peak lagged linear correlation — quick screen only |
...
Here each edge weight *is a probability* rather than a yes/no decision, and every link reports mean strength ± standard deviation:

- **Bootstrap edge probability** — re-runs discovery on block-resampled copies; the weight is the fraction of resamples containing the link
- **Stability selection (Lasso)** — selection frequency across random sub-samples
- **Bayesian regression posterior** — probability derived from the model's own coefficient uncertainty
...
| --- | --- | --- | --- | --- |
| Speed | Fast | Moderate | Slow | Moderate–slow |
...
No single method is authoritative. Running three or four from *different* families and trusting the links they agree on is the most reliable practice — methods resting on different assumptions rarely make the same mistake. The report's method-comparison table is built for exactly this.
...
- **Method comparison** — agreement matrix across families
...
- Causal graph and propagation plot share one visual language — glow-halo nodes, solid arrowheads, lag labels, left-to-right layered layout via Eades–Lin–Smyth cycle breaking
...
- Built-in synthetic demo process — the full pipeline is explorable without uploading anything
...
| Graph rendering | [Dash Cytoscape](https://dash.plotly.com/cytoscape) · [NetworkX](https://networkx.org/) | — |
...
| Causal discovery | [tigramite](https://github.com/jakobrunge/tigramite) (PCMCI) · [lingam](https://github.com/cdt15/lingam) (VarLiNGAM) | — |
...
The app is accessible at **[causal-rca-app.azurewebsites.net](https://causal-rca-app.azurewebsites.net)** — no installation or account required.
...
The application is built entirely in Python. Frontend and backend are unified in a single Dash application — Dash renders the UI as React components in the browser while the Python backend handles data processing, causal inference, and callback logic. Styling uses a custom CSS stylesheet with a cyan/violet design system (cyan for data and signal, violet for AI and inference) and CSS variables for theming.
...
- **Interventional analysis** — "what if this variable were held constant" simulation from the fitted structure
